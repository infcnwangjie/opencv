...
class DetectorHandle(object):
    ...
    def get_currentbag_position_withoutlandmark(self, original_img=None):
        ...
        if bags is not None or self.current_bag is not None:
            return None
    ...
    def check_suck(self, perspective_img=None, original_img=None, frame_index=0):
        ...
        if self.current_bag.suck_times > 5:
            self.current_bag.if_suck == True
            return True
        ...
        if perspective_img is  None:
            ...
            if bag_info is None or hock_info is None:
                ...
                if abs(hock_col - bag_col) < 20 and abs(
                        hock_row - bag_row) < 20:
                    self.current_bag.suck_frame_status[str(frame_index)]=True
                    self.current_bag.suck_times += 1

        else:
            # 地标定位成功之时，不过实际中else不会执行
            if self.current_bag is None or self.hock is None:
                return False
        ...
        if self.status_show is not None:
            self.status_show.showMessage("已经吸住" if self.current_bag.suck_times > 1 else "还没吸住")

        print(self.current_bag.suck_frame_status)
        print("已经吸住" if self.current_bag.suck_times > 3 else "还没吸住")
        return self.current_bag.suck_times > 3
    ...
    def suck_bag(self, perspective_img=None, original_img=None, frameindex=0):
        ...
        if self.current_bag is not None and self.current_bag.if_suck == True:
            return True
        ...
        if if_suck == True:
            if perspective_img is not None:
                cv2.putText(perspective_img, "suck bag success", (500, 600), cv2.FONT_HERSHEY_SIMPLEX, 1.2,
                            (255, 255, 255), 2)
            if original_img is not None:
                cv2.putText(original_img, "suck bag success", (500, 600), cv2.FONT_HERSHEY_SIMPLEX, 1.2,
                            (255, 255, 255), 2)
