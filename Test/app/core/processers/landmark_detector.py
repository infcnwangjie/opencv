...
class LandMarkDetecotr:
    ...
    def position_landmark(self):
        ...
        for landmark_roi in landmark_rois:
            ...
            # col = landmark.col
            # row = landmark.row
            col,row= real_position_dic[landmark_roi.label]
    ...
    def __generat_rect(self, dest=None):
        def warp_filter(c):
            '''内部过滤轮廓'''
            isbig = 90 <= cv2.contourArea(c) < 300
            rect_x, rect_y, rect_w, rect_h = cv2.boundingRect(c)
            return isbig and 5 < rect_w <= 30 and 5 < rect_h <= 30

        global rows, cols, step
        landmark_rois = self.__get_landmark_rois()
        half_totallandmark = int(len(landmark_rois) / 2)
        ...
        for roi_template in landmark_rois:
            ...
            for c in contours:
                ...
                if must_unique_key in must_unique_window:
                    ...
                    landmark_obj = NearLandMark(rect_x, rect_y, target[rect_y:rect_y + rect_h, rect_x:rect_x + rect_w])
...
if __name__ == '__main__':
    # 218  240  60
    src = LandMarkDetecotr(img=cv2.imread('c:/work/nty/hangche/2020-05-15-15-59-16test.bmp')).position_landmark()
