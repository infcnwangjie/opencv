...
class Box:
    ...
    def compute_iner_contours(self):
        if self.roi_contours is None and self.has_compute_contours == False:
            ...
            ret, thresh = cv2.threshold(roi_img, 110, 255, cv2.THRESH_BINARY_INV)  # 简单阈值
...
class LandMark(Box):
    ...
    def modify_box_content(self, no_num=True):
        ...
        # if len(digital_nums) > 1:
        #     self.box_content = "landmark:" + digital_nums[1] + "->(" + str(self.boxcenterpoint[0]) + "," + str(
        #         self.boxcenterpoint[1]) + ")"
        # else:
        #     self.box_content = "landmark:" + digital_nums[0] + "->(" + str(self.boxcenterpoint[0]) + "," + str(
        #         self.boxcenterpoint[1]) + ")"
