...
class SetRoiWidget(QWidget):
	...
	def init_ui(self):
		...
		self.pushButton_fetchavi = QPushButton('摄像头获取', self)
		self.pushButton_fetchavi.setCheckable(True)
		self.pushButton_fetchavi.setMaximumSize(QSize(100, 16777215))
		self.pushButton_fetchavi.clicked.connect(self.pushButton_fetchavi_clicked)
		self.gridLayout.addWidget(self.pushButton_fetchavi, 1, 0, 1, 1)
		...
		self.gridLayout.addWidget(self.pushButton_cut, 2, 0, 1, 1)
	...
	def pushButton_open_clicked(self):
		...
		self.graphicsView.set_picture(filename)


	def pushButton_fetchavi_clicked(self):


		try:
			haikangsdk = SdkHandle()
			self.picture = haikangsdk.read()
			# self.graphicsView = GraphicsView(picture=self.picture, parent=self)
			self.graphicsView.set_picture(self.picture)
		except Exception as e:
			filename, filetype = QFileDialog.getOpenFileName(self,
			                                                 "选取文件",
			                                                 "./",
			                                                 "All Files (*);;Text Files (*.txt)")
			self.picture=ImageProvider(videofile=filename).read()
			self.graphicsView.set_picture(self.picture)
