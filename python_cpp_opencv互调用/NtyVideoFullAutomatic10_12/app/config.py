...
load_config()
DETECT_BY_MULTIPLEAREA = int(config_dic['DETECT_BY_MULTIPLEAREA'])  # 多区域混合计算

DETECT_BY_HSVINRANGE = int(config_dic['DETECT_BY_HSVINRANGE'])  # 单一hsv阈值区间计算

DISTANCE_CAMERA_HOCK = int(config_dic['DISTANCE_CAMERA_HOCK'])  # 摄像头与钩子Y轴误差距离60cm

DEBUG = True if 'True' in config_dic['DEBUG'] else False  # 开启测试模式
IMG_WIDTH = int(config_dic['IMG_WIDTH'])  # 限定图像宽度
IMG_HEIGHT = int(config_dic['IMG_HEIGHT'])  # 限定图像高度

land_mark_left_start = int(config_dic['land_mark_left_start'])  # 115
land_mark_left_end = int(config_dic['land_mark_left_end'])  # 210
land_mark_right_start = int(config_dic['land_mark_right_start'])  # 510
land_mark_right_end = int(config_dic['land_mark_right_end'])  # 600
...
permissible_distance = int(config_dic['permissible_distance'])  # 允许钩子识别误差在20公分
move_tobag_retry_times = int(config_dic['move_tobag_retry_times'])  # 钩子向袋子移动尝试次数
hock_y_error = int(config_dic['hock_y_error'])  # 允许钩子识别误差在20公分
hock_x_error = int(config_dic['hock_x_error'])  # 允许钩子识别误差在20公分
direct_choice = config_dic['direct_choice']  # 仅测试Y轴或者X轴
...
MVCAMERACONTROLDLL_PATH = config_dic['MVCAMERACONTROLDLL_PATH']  # 海康威视图像动态连接库
...
VIDEO_DIR = config_dic['VIDEO_DIR']  # 海康视频路径
ROIS_DIR = config_dic['ROIS_DIR']  # 海康ROI存储路径
SAVE_VIDEO_DIR = config_dic['SAVE_VIDEO_DIR']  # 视频留存路径
PROGRAM_DATA_DIR = config_dic['PROGRAM_DATA_DIR']  # 其他数据存储路径
BAGROI_DIR = config_dic['BAGROI_DIR']  # 袋子ROI存储
HOCK_ROI = config_dic['HOCK_ROI']  # 钩子ROI
SUPPORTREFROI_DIR = config_dic['SUPPORTREFROI_DIR']
LOG_PATH = config_dic['LOG_PATH']  # 日志存储文档
PLC_COM = config_dic['PLC_COM']  # PLC端口号
...
def reload():
    ...
    global DETECT_BY_MULTIPLEAREA, DETECT_BY_HSVINRANGE, DISTANCE_CAMERA_HOCK, DEBUG, IMG_WIDTH, IMG_HEIGHT, \
        ...
        LOG_PATH, PLC_COM, LASTER_HOCK_DISTANCE, HOCK_DISTANCE, LANDMARK_THREHOLD_START, LANDMARK_THREHOLD_END
