...
class PointLocationService:
	...
	# 预处理
	def preprocess(self, colorlow=(61, 83, 31), colorhigh=(81, 255, 250)):
	...
	def computer_bags_location(self, digitdetector):
		binary = self.preprocess(colorlow=[120, 50, 50], colorhigh=[180, 255, 255])
		...
		for countour in contours:
			...
			if rect_w > 4 and rect_h > 5 and cv2.contourArea(countour) > 20:
		...
		# 遍历每个箱子，每个箱子在作为一个特征区域，再次寻找轮廓，再次检测目标
		for boxindex, rect in enumerate(moderatesize_rects):
			box = Box(rect, self.img, id=boxindex)

			box.modify_box_content(digitdetector)
	...
	def computer_landmarks_location(self, digitdetector):
		binary = self.preprocess(colorlow=(61, 83, 31), colorhigh=(81, 255, 250))
		contours, hierarchy = cv2.findContours(binary, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
		if contours is None or len(contours) == 0:
			return
		# 大小适中的轮廓，过小的轮廓被去除了
		...
		cv2.drawContours(self.img, moderatesize_countours, -1, (0, 0, 205), 1)
		# shapedetector = ShapeDetector()
		# contour_rects = [cv2.boundingRect(c) for c in moderatesize_countours]
		# 遍历每个箱子，每个箱子在作为一个特征区域，再次寻找轮廓，再次检测目标
		for boxindex, rect in enumerate(moderatesize_rects):
			box = Box(rect, self.img, id=boxindex, digitdetector=digitdetector)
			box.modify_box_content(digitdetector)
			print(box.box_content)
			print(box.boxcenterpoint)
			# 标记坐标和值
